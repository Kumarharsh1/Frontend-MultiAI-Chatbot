...
import { chatWithBackend } from '../utils/api';
...
const ChatInterface = ({ chatbotType, service, chatbotConfig, connectionError }) => {
  ...
  const getWelcomeMessage = (type) => {
    const welcomeMessages = {
      news: "Hello! I'm your News Assistant. Ask me about the latest news!",
      personal: "Hi! I'm your Personal Assistant. How can I help you today?",
      creative: "Greetings! I'm your Creative Assistant. Let's create something amazing!",
      technical: "Hello! I'm your Technical Assistant. Need coding help or technical guidance?"
    };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: "Sorry, backend is unreachable.",
        timestamp: new Date().toISOString(),
        error: true
      }]);
    } finally {
        ...
        <div className="p-3 bg-red-500/20 border-b border-red-500/30">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-red-300 text-sm">{connectionError || apiError}</span>
            </div>
            <button onClick={clearError} className="text-red-300 hover:text-white text-sm">Dismiss</button>
          </div>
        </div>
      )}
